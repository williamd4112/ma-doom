...
from policies import MACommPolicy, MACnnPolicy, MACnnSepPolicy, MACommSepCriticPolicy

NUM_PLAYERS = 2

def train(config, num_frames, seed, policy, lrschedule, num_cpu, ckpt, start_port=8000):
    ...
    return learn(policy_fn, env, seed, ckpt, total_timesteps=num_timesteps, lrschedule=lrschedule)
...
def main():
    ...
    parser.add_argument('--policy', help='Policy architecture', choices=['cnn', 'comm', 'commsep'], default='comm')
    parser.add_argument('--lrschedule', help='Learning rate schedule', choices=['constant', 'linear'], default='constant')
    parser.add_argument('--thousand_frames', help='How many frames to train (/ 1e3). '
        'This number gets divided by 4 due to frameskip', type=int, default=400)
    ...
    while True:
        ...
        policy=args.policy, lrschedule=args.lrschedule, num_cpu=8, ckpt=current_ckpt, start_port=args.port)
        time.sleep(3)
